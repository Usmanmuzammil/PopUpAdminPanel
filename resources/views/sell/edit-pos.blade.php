...
<body>
    @foreach ($bill as $bill)
        <input type="hidden" name="update_bill_id" value="{{ $bill->id }}">
            ...
            <div class="col-md-6 bg-white">
                ...
                <div class="row mt-3">
                    <div class="col-md-4">
                        ...
                        <select name="booker_id" id="booker_id" class="form-control">
                            @foreach ($booker as $booker)
                            <option value="{{ $booker->id }}" {{ ($bill->booker_id==$booker->id)?"selected":"" }}>{{ $booker->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <div class="input-group">
                            <select class="form-select customer" name="order_type" id="order_type"
                                ...
                                <option value="dining" {{ ($bill->order_type=="dining")?"selected":"" }}>Dining</option>
                                <option value="delivery" {{ ($bill->order_type=="delivery")?"selected":"" }}>Delivery</option>
                                <option value="parcel" {{ ($bill->order_type=="parcel")?"selected":"" }}>Disposible Parcel</option>
                                <option value="take_away" {{ ($bill->order_type=="take_away")?"selected":"" }}>Take Away</option>
                                ...
                                <div id="mCSB_2" class="mCustomScrollBox mCS-light mCSB_vertical mCSB_inside"
                                    ...
                                    <div id="mCSB_2_container" class="mCSB_container"
                                        ...
                                        <table id="myTable"
                                            ...
                                            <tbody id="table">
                                                @foreach ($bill->getBillDetail as $value)
                                                <tr class='' data-id="{{ $value->product_id }}" >
                                                    <td class='w-100 ' colspan='3' ><span class='fw-bold'>{{ $value->getProduct->product_name }}</span><br>
                                                        <?php
                            echo $value->displayVariants($value->displayVariants());
                            echo $value->displayExtras($value->displayExtras())."<br>";
                            echo $value->displayAddons($value->displayAddons());
                            ?>



                                                        @if (!empty($value->varriantion) && is_array(json_decode($value->varriantion)))
                                                            @foreach (json_decode($value->varriantion) as $variant)
                                                                <span class="varriation_detail">
                                                                    <input type="hidden" class="selected_attribute_id" value="{{ $variant->attibute_id }}">
                                                                    <input type="hidden" class="selected_attribute_name" value="{{ $variant->attribute_name }}">
                                                                    <input type="hidden" class="selected_varriation_id" value="{{ $variant->varriation_id }}">
                                                                    <input type="hidden" class="selected_varriation_name" value="{{ $variant->varriation_name }}">
                                                                    <input type="hidden" class="selected_varriation_price" value="{{ $variant->varriation_price }}">
                                                                </span>
                                                            @endforeach
                                                        @endif
...
<!-- Display addons -->
<!-- Display addons -->
@if (!empty($value->addons) && is_array(json_decode($value->addons)))
    @foreach (json_decode($value->addons) as $addon)
        <span class="addon_detail">
            <input type="hidden" class="selected_addon_id" value="{{ $addon->addon_id }}">
            <input type="hidden" class="selected_addon_name" value="{{ $addon->addon_name }}">
            <input type="hidden" class="selected_addon_price" value="{{ $addon->addon_price }}">
        </span>
    @endforeach
@endif

<!-- Display extras -->
@if (!empty($value->extras) && is_array(json_decode($value->extras)))
    @foreach (json_decode($value->extras) as $extra)
        <span class="extra_detail">
            <input type="hidden" class="selected_extra_id" value="{{ $extra->extra_id }}">
            <input type="hidden" class="selected_extra_name" value="{{ $extra->extra_name }}">
            <input type="hidden" class="selected_extra_price" value="{{ $extra->extas_price }}">
        </span>
    @endforeach
@endif
                                                    ...
                                                    <td><input type='number' class='product_price form-control text-center' value="{{ $value->price }}"></td>
                                                    <td class='w-auto'><div class='text-center'><div class='input-step step-danger text-center'><button type='button' class='minus'>–</button><input type='number' class='product_qty' value="{{ $value->qty }}"><button type='button' style='background:#42ba96;' class='plus'>+</button></div></div></td>
                                                    <td class='w-auto'><input type='number' class='product_discount form-control ' value='0'></td>
                                                    <th class='sub_total product_subtotal text-center'>{{ $value->price }}</th>
                                                    <td ><a href='#' data-product_id="{{ $value->product_id }}" class='remove_btn btn btn-danger btn-sm'>X</td>
                                                    </tr>
                                                @endforeach
                                            </tbody>
                                        </table>
                    ...
                    <div class="col-md-4">
                        <div class="rounded payment-amount bg-primary">
                            <h4 class="text-white mx-2 py-2">Grand Total : <strong id="grand_total"
                                    class="net_total">{{ $bill->total }}</strong></h4>
                        </div>

                    </div>
                    <div class="col-md-4">
                        <input type="text" value="{{ $bill->discount }}" name="" id="discount" class="form-control" placeholder="Enter Discount">
                    </div>
                    <div class="col-md-4">
                        <div class="rounded payment-amount bg-primary">
                            <h4 class="text-white mx-2 py-2">NetTotal : <span id="net_total">{{ $bill->net_total }}</span></h4>
                        </div>
                    </div>
            ...
            <div class="col-md-5 ">
                <header class="bg-white p-2">
                    <div class="d-flex justify-content-between">
                        ...
                        <button type="button" class="btn btn-primary  payment-btn text-white float-end" id="finalize_sale"><i class="las la-money-bill-wave"></i>
                            Cash Bill</button>

                        <div class="dropdown d-inline-block">
                            <button class="menu-btn btn btn-outline-primary dropdown" id="finalize" type="button"><i class="fa fa-bars"> </i></button>
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="col-6">
                            ...
                            <input type="number" value="{{ $bill->paid_amount }}" name="paying_amount" id="paying_amount"
                                placeholder="Enter paying amount" class="form-control" required>
                        </div>
                        <div class="col-6">
                            <label for="totalamount">Total Amount</label>
                            <input type="number" value="{{ $bill->paid_amount }}" name="totalamount" id="modal_total" placeholder="Total Amount"
    ...
    <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button style="width: 100%;" id="add-product" data-dismiss="modal" type="button"
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            $.each(products, function(index, product) {
                ...
                html += '<div  class="col-lg-2 col-md-2" data-bs-toggle="modal" data-bs-target="#exampleModal" data-bs-whatever="@mdo"  >';
    ...
    $('#search_item').on('input', function () {
        ...
        if (searchItem) {
            ...
            matchedProducts.forEach(function (product) {
                html += "<a href='#' class='bg-primary text-white list-group-item search_product' data-id='" + product.id + "' data-name='" + product.product_name + "' data-price='" + product.selling_price + "' data-stock='" + product.stock + "' data-qty='" + product.opening_stock + "'>"  + product.product_code + ' - ' + product.product_name + '</span>'+ "</a>";
            });
...
'discout':disc
    ...
    if(rowCount==0){
       ...
       // var rem=$('#remaining').text();
        ...
        var update_bill_id=$('input[name=update_bill_id]').val();
...
var addons = {};
    $('.addon-check-box:checked').each(function () {
        ...
        addon_id = $(this).closest('.col-2').find('.addon_id_field').val();
        addons_name = $(this).closest('.col-2').find('.addon_name_field').val();
        addons_price = $(this).closest('.col-2').find('.addon_price_field').val();
        // Add the variant to the respective attribute in the object
    ...
    $('.extras-checkbox:checked').each(function () {
        ...
        var attribute_name = $(this).closest('.row').find('.attribute_name').text();
        extras_id = $(this).closest('.col-2').find('.extra_id_field').val();
        extras_name = $(this).closest('.col-2').find('.extra_name_field').val();
        extras_price = $(this).closest('.col-2').find('.extra_price_field').val();
        // Add the variant to the respective attribute in the object
...
// Function to create a unique identifier for items
// function createItemIdentifier(product_id, selectedVariants, selectedAddons, selectedExtras) {
//     var identifier = product_id;

//     if (selectedVariants.length > 0) {
//         identifier += '{' + selectedVariants.join(',') + '}';
//     }

//     if (selectedAddons.length > 0) {
//         identifier += '{Addons:[' + selectedAddons.join(',') + ']}';
//     }

//     if (selectedExtras.length > 0) {
//         identifier += '{Extras:[' + selectedExtras.join(',') + ']}';
//     }

//     return identifier;
// }

// // Function to update the quantity and subtotal for an item
// function updateItemQuantityAndSubtotal(itemRow, quantity) {
//     var price = parseFloat(itemRow.find('.product_price').val()) || 0;
//     var subtotal = price * quantity;
//     itemRow.find('.product_qty').val(quantity);
//     itemRow.find('.sub_total').text(subtotal.toFixed(2));
//     grandTotal();
// }
...
// $('#add-product').click(function () {
//     var item_total_price = parseFloat($(this).closest('.modal').find('.product_price').val()) || 0;
//     var product_id = $(this).closest('.modal').find('.product_id').val();
//     var product_name = $(this).closest('.modal').find('.product_name').val();
//     var product_price = parseFloat($(this).closest('.modal').find('.product_price').val()) || 0;

//     var selectedVariants = [];
//     var selectedAddons = [];
//     var selectedExtras = [];

//     // Handle selection attributes (radio buttons and checkboxes)
//     $('.variants-section .variant-selection input[type="radio"]:checked').each(function () {
//         var attribute_name = $(this).closest('.variants-container').find('.attribute_name_field').val();
//         var variant_name = $(this).closest('.variant-selection').find('.single_varriantion_name_field').val();
//         var attribute_type = 'single';

//         var attributeNameValue = attribute_name + ': ' + variant_name;

//         if (attribute_type === 'single') {
//             selectedVariants.push(attributeNameValue);
//         }
//     });

//     // Handle selection attributes (checkboxes)
//     $('.variants-section .variant-selection input[type="checkbox"]:checked').each(function () {
//         var attribute_name = $(this).closest('.variants-container').find('.attribute_name_field').val();
//         var variant_name = $(this).closest('.variant-selection').find('.varriantion_name_field').val();
//         var attribute_type = 'multi';

//         var attributeNameValue = attribute_name + ': ' + variant_name;

//         if (attribute_type === 'multi' || selectedVariants.indexOf(attributeNameValue) === -1) {
//             selectedVariants.push(attributeNameValue);
//         }
//     });

//     // Handle addons
//     $('.addons-section .addon-check-box:checked').each(function () {
//         var addon_name = $(this).closest('.addons-section').find('.attribute_name_field').val();
//         var variant_name = $(this).closest('.addon').find('.single_varriantion_name_field').val();
//         var attributeNameValue = addon_name + ': ' + variant_name;
//         selectedAddons.push(attributeNameValue);
//     });

//     // Handle extras
//     $('.extras-checkbox:checked').each(function () {
//         var extra_name = $(this).closest('.extras-checkbox').find('.attribute_name_field').val();
//         var variant_name = $(this).closest('.extras-checkbox').find('.single_varriantion_name_field').val();
//         var attributeNameValue = extra_name + ': ' + variant_name;
//         selectedExtras.push(attributeNameValue);
//     });

//     var variantsHtml = '';
//     if (selectedVariants.length > 0) {
//         variantsHtml += "Variants: " + selectedVariants.join(', ') + "<br>";
//     }

//     var addonsHtml = '';
//     if (selectedAddons.length > 0) {
//         addonsHtml += "Addons: " + selectedAddons.join(', ') + "<br>";
//     }

//     var extrasHtml = '';
//     if (selectedExtras.length > 0) {
//         extrasHtml += "Extras: " + selectedExtras.join(', ') + "<br>";
//     }

//     // Check if the item has any attributes or selections
//     if (selectedVariants.length > 0 || selectedAddons.length > 0 || selectedExtras.length > 0) {
//         var itemIdentifier = createItemIdentifier(product_id, selectedVariants, selectedAddons, selectedExtras);

//         // Check if an item with the same identifier already exists in the table
//         var existingItems = $('#table tr[data-id^="' + product_id + '"]');
//         var found = false;

//         existingItems.each(function () {
//             var existingItem = $(this);
//             var existingIdentifier = existingItem.data('id');

//             if (existingIdentifier === itemIdentifier) {
//                 // Item with the same identifier already exists, so increment the quantity
//                 var currentQuantity = parseInt(existingItem.find('.product_qty').val()) || 0;
//                 updateItemQuantityAndSubtotal(existingItem, currentQuantity + 1);
//                 found = true;
...
//         if (!found) {
//             // Item with the same identifier does not exist, add it as a new item
//             $('#table').append(
//                 "<tr data-id='" + itemIdentifier + "'>\
//                 <td colspan='3'><span class='fw-bold'>" + product_name + "</span><br>\
//                 " + variantsHtml + addonsHtml + extrasHtml + "</td>\
//                 <td><input type='number' class='product_price form-control text-center' value='" + product_price + "'></td>\
//                 <td><input type='number' class='product_qty form-control text-center' value='1'></td>\
//                 <td><input type='number' class='product_disc form-control text-center' value='0'></td>\
//                 <td class='sub_total text-center'>" + item_total_price.toFixed(2) + "</td>\
//                 <td><a href='#' class='remove_btn btn btn-danger btn-sm'>X</a></td>\
//                 </tr>"
//             );
//         }
//     } else {
//         // No attributes or selections, check if the product already exists in the table
//         var existingProduct = $('#table tr[data-id="' + product_id + '"]');
//         if (existingProduct.length > 0) {
//             // Product already exists, update its quantity
//             var currentQuantity = parseInt(existingProduct.find('.product_qty').val()) || 0;
//             updateItemQuantityAndSubtotal(existingProduct, currentQuantity + 1);
//         } else {
//             // Product does not exist, add it as a new item with default quantity
//             $('#table').append(
//                 "<tr data-id='" + product_id + "'>\
//                 <td colspan='3'><span class='fw-bold'>" + product_name + "</span></td>\
//                 <td><input type='number' class='product_price form-control text-center' value='" + product_price + "'></td>\
//                 <td><input type='number' class='product_qty form-control text-center' value='1'></td>\
//                 <td><input type='number' class='product_disc form-control text-center' value='0'></td>\
//                 <td class='sub_total text-center'>" + item_total_price.toFixed(2) + "</td>\
...
// $('#table').on('input', '.product_qty', function () {
//     var itemRow = $(this).closest('tr');
//     var quantity = parseInt($(this).val()) || 0;
//     updateItemQuantityAndSubtotal(itemRow, quantity);
// });

// Function to create a unique identifier for items

[{"attibute_id":"1","attribute_name":"Size","varriation_price":"0","varriation_id":null,"varriation_name":"ch.pasta full"},
{"attibute_id":"2","attribute_name":"Flavour","varriation_price":"50","varriation_id":null,"varriation_name":"white Souce Ch.pasta"}]
